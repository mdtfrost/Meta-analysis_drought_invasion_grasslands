...
#for desktop
setwd("/Volumes/Morgan_HD/metaanalysis_droughtinvasion")
search()
...
meta_rr <- meta %>%
  ...
  mutate(inv_nat = ifelse(inv_nat == "i", "introduced", ifelse(inv_nat == "n", "native", 0)))
...
meta_rr_v <- meta_rr %>%
  mutate(vi_dr = (drought_sd^2 / (drought_n * drought_avg^2)) + (control_sd^2 / (control_n * control_avg^2))) %>%
  drop_na(vi_dr) %>%
  filter(vi_dr != "Inf") %>%
  filter(vi_dr > 0)

# lnrr_dr = effect sizes (log response ratio)
# vi = variance of effect sizes 
# inv_nat = native vs. introduced (can be used as a moderator later)
# study_ID = random effect

# Basic random-effects model
res_overall <- rma(yi = lnrr_dr, vi = vi_dr, data = meta_rr_v, method = "REML")

# View results including Q, I², τ²
summary(res_overall)
...
tuk_drought_len2 <- emmeans(drought_lmer_len, ~ drought_length)
contrast(tuk_drought_len2, "consec", simple = "each", combine = TRUE, adjust = "mvt")  #within 2-6 months, n-i p = 0.0049
...
##################################################
###### reorder and revise figures for JOE revision #########
...
inv_nat_fig <- ggplot(data = meta_rr_inv_nat, aes(x = avg, y = inv_nat, color = inv_nat)) + 
  geom_vline(aes(xintercept = 0), linewidth = 0.8, linetype = "dashed") +
  geom_errorbarh(aes(xmin = lower, xmax = upper), linewidth = 1, height = 0.15, color = "black")+
  geom_point(size = 4) + 
  xlim(-0.75, 0.25) +
  scale_color_manual(values = cbPalette) + 
  labs(x = "Effect Size", y = "Plant Status", color = "Plant Status") +
  theme(legend.title = element_text(size = 18), panel.border = element_rect(linewidth = 2)) +
  guides(color = guide_legend(reverse = TRUE))

#export
pdf(file = "Figure2_v2.pdf", width = 8, height = 4)
inv_nat_fig
dev.off()
...
resp_fig2 <- ggplot(data = meta_rr_resp2, aes(x = avg, y = resp_type2, color = inv_nat)) + 
  geom_vline(aes(xintercept = 0), linewidth = 0.8, linetype = "dashed") +
  geom_errorbarh(aes(xmin = lower, xmax = upper), linewidth = 1, height = 0.2, color = "black")+
  geom_point(size = 4) + 
  xlim(-1.5, 0.5) +
  scale_color_manual(values = cbPalette) + 
  labs(x = "", y = "Response Variable", color = "Plant Status") +
  theme(legend.title = element_text(size = 18), panel.border = element_rect(linewidth = 2)) +
  guides(color = guide_legend(reverse = TRUE))
...
study_fig <- ggplot(data = meta_rr_study, aes(x = avg, y = study_type, color = inv_nat)) + 
  ...
  geom_point(size = 4) + 
  xlim(-1.5, 0.5) +
  theme(panel.border = element_rect(linewidth = 2), legend.position = "none") +
  scale_color_manual(values = cbPalette) + 
  labs(x = "", y = "Study Type", color = "Plant Status") 

#3c) drought implementation
meas_fig <- ggplot(data = meta_rr_meas, aes(x = avg, y = measure_of_drought, color = inv_nat)) + 
  ...
  theme(panel.border = element_rect(linewidth = 2), legend.position = "none") +
  scale_color_manual(values = cbPalette) + 
  labs(x = "", y = "Drought\nImplementation", color = "Plant Status")
...
dec_fig <- ggplot(data = meta_rr_dec, aes(x = avg, y = mag_drt_dec, color = inv_nat)) + 
  ...
  theme(panel.border = element_rect(linewidth = 2), legend.position = "none") +
  scale_color_manual(values = cbPalette) + 
  labs(x = "", y = "Percent Decrease\nin Water (%)", color = "Plant Status")
...
length_fig <- ggplot(data = meta_rr_length, aes(x = avg, y = drought_length, color = inv_nat)) + 
  ...
  theme(panel.border = element_rect(linewidth = 2), legend.position = "none") +
  scale_color_manual(values = cbPalette) + 
  labs(x = "", y = "Drought Duration", color = "Plant Status")

#3g)functional groups
...
fig3 <- resp_fig2 + study_fig + meas_fig + arid_fig + dec_fig + length_fig + func_fig + plot_layout(ncol = 1)

#export
pdf(file = "Figure3_v2.pdf", width = 12, height = 26)
fig3
dev.off()
...
fig4 <- inv_nat_fig_dr2 + inv_nat_fig_drdriv + plot_layout(ncol = 1)

#export
pdf(file = "Figure4_v2.pdf", width = 8, height = 8)
fig4
dev.off()
...
################################################
####### JOE revision round 2 ###########
#figure 4 redo as one
# Combine both datasets
meta_rr_inv_nat_dr$context <- "drought only"
meta_rr_inv_nat_drdriv$context <- "drought + global driver"
fig4_df <- rbind(meta_rr_inv_nat_dr, meta_rr_inv_nat_drdriv)
fig4_df$group <- paste(fig4_df$inv_nat, "(", fig4_df$context, ")", sep = "")

fig4 <- ggplot(fig4_df, aes(x = avg, y = group, color = inv_nat)) + 
  geom_vline(xintercept = 0, linewidth = 0.8, linetype = "dashed") +
  geom_errorbarh(aes(xmin = lower, xmax = upper), linewidth = 1, height = 0.2, color = "black") +
  geom_point(size = 4) +
  xlim(-0.8, 0.2) +
  scale_color_manual(values = cbPalette) +
  theme(panel.border = element_rect(linewidth = 2)) +
  labs(x = "Effect Size", y = "Plant Status and Treatment Context", color = "Plant Status") +
  guides(color = guide_legend(reverse = TRUE))
#Figure4_v3 800x800

#export
pdf(file = "Figure4_v3.pdf", width = 15, height = 7)
fig4
dev.off()
